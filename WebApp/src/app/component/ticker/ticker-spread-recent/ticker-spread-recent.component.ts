...
@Component({
  selector: 'app-ticker-spread-recent',
  templateUrl: './ticker-spread-recent.component.html',
  styleUrls: ['./ticker-spread-recent.component.scss']
})
export class TickerSpreadRecentComponent implements OnInit {
  ...
  public async get() {
    ...
    try {
      const payload = await superagent.get(`${env.api}/spread/recent?symbol=${this.ticker.symbol}`).withCredentials();
      this.spreads = payload.body.response;
      console.log('Recent Spreads', this.spreads);
    } catch (error) {
